...
class CustomerControllerWebMvcTest {
  ...
  void testGetCustomers() throws Exception {
    ...
    doReturn(CompletableFuture.completedFuture(List.of(customer)))
        .when(queryGateway)
        .query(any(GetCustomersQuery.class), any(ResponseType.class));

    MvcResult mvcResult =
        mockMvc.perform(get("/api/customers")).andExpect(request().asyncStarted()).andReturn();
  ...
  void testGetCustomer() throws Exception {
    ...
    doReturn(CompletableFuture.completedFuture(customer))
        .when(queryGateway)
        .query(any(GetCustomerQuery.class), any(Class.class));

    MvcResult mvcResult =
        mockMvc.perform(get("/api/customers/c1")).andExpect(request().asyncStarted()).andReturn();
  ...
  void testAddCustomer() throws Exception {
    doReturn(CompletableFuture.completedFuture("c1"))
        .when(commandGateway)
        .send(any(AddCustomerCommand.class));
  ...
  void testUpdateCustomer() throws Exception {
    doReturn(CompletableFuture.completedFuture("c1"))
        .when(commandGateway)
        .send(any(UpdateCustomerCommand.class));
  ...
  void testGetCustomerOrders() throws Exception {
    ...
    doReturn(CompletableFuture.completedFuture(List.of(order)))
        .when(queryGateway)
        .query(any(GetCustomerOrdersQuery.class), any(ResponseType.class));

    MvcResult mvcResult =
        mockMvc
            .perform(get("/api/customers/c1/orders"))
            .andExpect(request().asyncStarted())
            .andReturn();
