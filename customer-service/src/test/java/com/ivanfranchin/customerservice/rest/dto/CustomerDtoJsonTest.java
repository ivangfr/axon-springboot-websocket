...
class CustomerDtoJsonTest {
    ...
    @Autowired
    private JacksonTester<AddCustomerRequest> addCustomerRequestJson;

    @Autowired
    private JacksonTester<UpdateCustomerRequest> updateCustomerRequestJson;

    @Autowired
    private JacksonTester<CustomerResponse> customerResponseJson;

    @Autowired
    private JacksonTester<CustomerOrderResponse> customerOrderResponseJson;

    @Autowired
    private JacksonTester<CustomerAddedEvent> customerAddedEventJson;

    @Autowired
    private JacksonTester<CustomerUpdatedEvent> customerUpdatedEventJson;

    @Autowired
    private JacksonTester<CustomerDeletedEvent> customerDeletedEventJson;

    @Autowired
    private JacksonTester<RestaurantAddedEvent> restaurantAddedEventJson;

    @Autowired
    private JacksonTester<RestaurantUpdatedEvent> restaurantUpdatedEventJson;

    @Autowired
    private JacksonTester<RestaurantDeletedEvent> restaurantDeletedEventJson;

    @Autowired
    private JacksonTester<RestaurantDishAddedEvent> restaurantDishAddedEventJson;

    @Autowired
    private JacksonTester<RestaurantDishUpdatedEvent> restaurantDishUpdatedEventJson;

    @Autowired
    private JacksonTester<RestaurantDishDeletedEvent> restaurantDishDeletedEventJson;
    ...
    void testCustomerOrderResponseSerialization() throws Exception {
        CustomerOrderResponse.OrderItem item = new CustomerOrderResponse.OrderItem("Pizza", new BigDecimal("12.75"), (short) 2);
        CustomerOrderResponse response = new CustomerOrderResponse("o1", "Pizza Place", "CREATED",
                new BigDecimal("25.50"), ZonedDateTime.parse("2025-01-15T10:30:00Z"), Set.of(item));
    ...
    void testRestaurantDishAddedEventSerialization() throws Exception {
        RestaurantDishAddedEvent event = new RestaurantDishAddedEvent("r1", "d1", "Pizza Margherita", new BigDecimal("6.99"));
    ...
    void testRestaurantDishUpdatedEventSerialization() throws Exception {
        RestaurantDishUpdatedEvent event = new RestaurantDishUpdatedEvent("r1", "d1", "Pizza Margherita 35cm", new BigDecimal("7.99"));
