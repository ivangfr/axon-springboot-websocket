...
function connectToWebSocket() {
  ...
  stompClient.onWebSocketClose = function () {
    showModal(
      ...
      "WebSocket is disconnected. Maybe, customer-service is down or restarting",
    );
    setWsStatus(false);
  };
...
function addCustomer(customer) {
  const row =
    '<div id="' +
    customer.id +
    '" class="customer-card bg-white rounded-xl shadow-sm border border-gray-200 p-4">' +
    ...
    '<h3 class="name text-base font-semibold text-gray-900">' +
    customer.name +
    "</h3>" +
    ...
    "</div>" +
    "</div>" +
    '<p class="address text-sm text-gray-600 mb-3">Address: <strong>' +
    customer.address +
    "</strong></p>" +
...
function getOrderRow(order) {
  ...
  return (
    ...
    order.status +
    "</td>" +
    '<td class="py-1.5 px-2"><strong>' +
    order.restaurantName +
    "</strong></td>" +
    '<td class="py-1.5 px-2 text-right font-medium">' +
    accounting.formatMoney(order.total) +
    "</td>" +
    '<td class="py-1.5 px-2"><ul class="list-disc list-inside text-xs text-gray-600">' +
    items +
    "</ul></td>" +
    "</tr>"
  );
...
function validateAndGetFormData() {
  ...
  if (name.trim().length === 0 || address.trim().length === 0) {
    showModal(
      $("#alertModal"),
      "Missing fields",
      "Please inform customer Name and Address",
    );
    return null;
  }
