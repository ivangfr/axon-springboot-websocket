...
function connectToWebSocket() {
  ...
  stompClient.onWebSocketClose = function () {
    showModal(
      ...
      "WebSocket is disconnected. Maybe, restaurant-service is down or restarting",
    );
    setWsStatus(false);
  };
...
function addRestaurant(restaurant) {
  const row =
    '<div id="' +
    restaurant.id +
    '" class="restaurant-card bg-white rounded-xl shadow-sm border border-gray-200 p-4">' +
...
function getRestaurantDishRow(dish) {
  return (
    ...
    dish.dishId +
    "</td>" +
    '<td class="py-1.5 px-2 name"><strong>' +
    dish.dishName +
    "</strong></td>" +
    '<td class="py-1.5 px-2 price">' +
...
function addRestaurantDish(dish) {
  $("#" + dish.restaurantId)
    .find("tbody")
    .prepend(getRestaurantDishRow(dish));
}
...
function validateAndGetRestaurantDishFormData() {
  const restaurantId = $(
    '#restaurantDishForm input[name="restaurantId"]',
  ).val();
  ...
  if (dishName.trim().length === 0 || dishPrice <= 0) {
    showModal(
      $("#alertModal"),
      "Missing fields",
      "Please inform dish Name and Price",
    );
    return null;
  }
  ...
  const orders = data
    .map((order) => {
      ...
      return (
        ...
        "</td>" +
        '<td class="py-1.5 px-3"><ul class="list-disc list-inside text-xs text-gray-600">' +
        items +
        "</ul></td>" +
        "</tr>"
      );
...
$(function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        $modal
          .find(".modal-content")
          .empty()
          .append(buildRestaurantOrderTable(data));
        $modal.removeClass("hidden");
      },
    });
  });
  ...
  $('#restaurantDishForm button[name="btnSave"]').click(function (event) {
    ...
    let type = "POST";
    let url = restaurantServiceApiBaseUrl.concat(
      "/",
      restaurantDishData.restaurantId,
      "/dishes",
    );
  ...
  $("#restaurantList").on("click", ".btnDeleteDish", function () {
    ...
    showModal(
      ...
      function () {
        $.ajax({
          type: "DELETE",
          url: restaurantServiceApiBaseUrl.concat(
            "/",
            restaurantId,
            "/dishes/",
            dishId,
