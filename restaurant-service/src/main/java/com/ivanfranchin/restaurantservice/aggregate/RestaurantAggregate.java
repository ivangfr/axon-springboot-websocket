...
public class RestaurantAggregate {
  ...
  public void handle(UpdateRestaurantDishCommand command) {
    this.dishes.stream()
        .filter(d -> d.getId().equals(command.getDishId()))
        .findAny()
        .ifPresentOrElse(
            d -> {
              String newName =
                  MyStringUtils.getTrimmedValueOrElse(command.getDishName(), d.getName());
              BigDecimal newPrice =
                  command.getDishPrice() == null ? d.getPrice() : command.getDishPrice();
              AggregateLifecycle.apply(
                  new RestaurantDishUpdatedEvent(
                      command.getRestaurantId(), command.getDishId(), newName, newPrice));
            },
            () -> {
  ...
  public void handle(RestaurantDishUpdatedEvent event) {
    this.id = event.getRestaurantId();
    this.dishes.stream()
        .filter(d -> d.getId().equals(event.getDishId()))
        .findAny()
        .ifPresent(
            d -> {
  ...
  public void handle(DeleteRestaurantDishCommand command) {
    ...
    AggregateLifecycle.apply(
        new RestaurantDishDeletedEvent(command.getRestaurantId(), command.getDishId()));
  }
