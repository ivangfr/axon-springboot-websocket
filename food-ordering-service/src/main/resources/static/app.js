...
function addRestaurant(restaurant) {
    ...
    const card =
        '<div class="restaurant-card px-4 py-3 bg-white border border-gray-200 rounded-lg cursor-pointer hover:border-teal-400 hover:shadow-sm transition-all flex items-center justify-between" data-id="'+restaurant.id+'">'+
...
function updateRestaurant(restaurant) {
    dataStore.restaurants[restaurant.id] = restaurant
    $('.restaurant-card[data-id="'+restaurant.id+'"]').find('span:first').text(restaurant.name)
}
...
function addRestaurantDish(dish) {
    ...
    if (wizardState.currentStep === 3 && wizardState.restaurantId === dish.restaurantId) {
        renderDishRows()
        updateStep3Total()
    }
}
...
function updateRestaurantDish(dish) {
    ...
    if (dishList) {
        ...
        if (idx >= 0) {
            dishList[idx] = {dishId: dish.dishId, dishName: dish.dishName, dishPrice: dish.dishPrice}
        }
    }
    if (wizardState.currentStep === 3 && wizardState.restaurantId === dish.restaurantId) {
        renderDishRows()
...
function removeRestaurantDish(dish) {
    const dishList = dataStore.dishes[dish.restaurantId]
    if (dishList) {
        dataStore.dishes[dish.restaurantId] = dishList.filter(d => d.dishId !== dish.dishId)
    }
    if (wizardState.currentStep === 3 && wizardState.restaurantId === dish.restaurantId) {
        renderDishRows()
        updateStep3Total()
...
function renderDishRows() {
    ...
    dishes.forEach(dish => {
        $tbody.append(
            '<tr id="'+dish.dishId+'" class="border-b border-gray-100 dish-row">'+
                '<td class="py-2 px-2 text-center">'+
                    '<input type="checkbox" class="dish-checkbox rounded border-gray-300 text-teal-500 focus:ring-teal-500">'+
                '</td>'+
                '<td class="py-2 px-2 name text-sm">'+dish.dishName+'</td>'+
                '<td class="py-2 px-2 price text-sm text-gray-600">'+accounting.formatMoney(dish.dishPrice)+'</td>'+
                '<td class="py-2 px-2">'+
...
function updateStep3Total() {
    let total = 0
    $('#dishTableBody').find('.dish-row').each(function() {
        const $tr = $(this)
...
function updateStep3Next() {
    ...
    $('#dishTableBody').find('.dish-checkbox').each(function() {
...
function renderReview() {
    ...
    items.forEach(item => {
        ...
        total += lineTotal
        $list.append('<li class="text-sm flex justify-between"><span>'+item.quantity+'x '+item.dishName+'</span><span class="text-gray-600">'+accounting.formatMoney(lineTotal)+'</span></li>')
    })
...
function goToStep(step) {
    ...
    $('.step-circle').each(function() {
        ...
        if (s < step) {
            $(this).removeClass('bg-gray-200 text-gray-500').addClass('bg-teal-500 text-white')
            ...
            $(this).removeClass('bg-teal-500 text-white').addClass('bg-gray-200 text-gray-500')
            $(this).find('.step-number').removeClass('hidden')
            $(this).find('.step-check').addClass('hidden')
        }
    })
    ...
    $('.step-label').each(function() {
        const s = $(this).data('step')
        $(this).toggleClass('text-teal-600', s <= step).toggleClass('text-gray-400', s > step)
    })

    $('.step-line').each(function() {
        const from = $(this).data('from')
        $(this).toggleClass('bg-teal-500', from < step).toggleClass('bg-gray-300', from >= step)
    })
...
function resetWizard() {
    ...
    $('#customerDropdown').find('.dropdown-text').text('Select Customer').removeClass('text-gray-900').addClass('text-gray-500')
    $('#customerDropdown').find('.dropdown-value').val('')
    $('.restaurant-card').removeClass('border-teal-500 bg-teal-50').addClass('border-gray-200')
...
$(function () {
    ...
    $('#step1Next').click(function() { goToStep(2) })
    $('#step2Back').click(function() { goToStep(1) })
    $('#step2Next').click(function() { goToStep(3) })
    $('#step3Back').click(function() { goToStep(2) })
    $('#step3Next').click(function() { goToStep(4) })
    $('#step4Back').click(function() { goToStep(3) })
    ...
    $('#btnPlaceOrder').click(function() {
        const items = getSelectedItemsFromStep3()
        if (items.length === 0) {
            showModal($('#alertModal'), 'No dishes selected', 'Please select at least one dish.')
            return
        }
        if (!wizardState.customerId) {
            showModal($('#alertModal'), 'No customer selected', 'Please select a customer.')
            return
        }
        ...
        $.ajax({
            ...
            success: function(data) {
                showModal($('#alertModal'), 'Order Submitted', 'Order submitted successfully. The order id is "' + data + '"', null, function() {
                    resetWizard()
                })
            }
        })
