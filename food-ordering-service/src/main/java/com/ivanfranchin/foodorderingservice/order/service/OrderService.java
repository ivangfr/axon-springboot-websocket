...
public class OrderService {
  ...
  public CompletableFuture<String> createOrder(CreateOrderRequest request) {
    ...
    Restaurant restaurant =
        restaurantService.validateAndGetRestaurant(request.restaurantId().toString());
    Set<OrderItem> items =
        request.items().stream()
            .map(
                i -> {
                  Dish dish =
                      restaurantService.validateAndGetRestaurantDish(
                          restaurant.getId(), i.dishId().toString());
